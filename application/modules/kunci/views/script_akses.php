...
<script type="text/javascript">
    ...
    var table_akses = $('#table-akses').DataTable( {"bAutoWidth": false,
        ajax: {
            processing: true,
            serverSide: true,
            url: '<?php echo site_url('kunci/table_akses'); ?>',
            type: 'POST',
            data: function(d) {
    ...
    function DeleteDataRakor(id) {
        bootbox.confirm("Apakah Anda yakin untuk menghapus data ini ?", function(result) {
            ...
            if (result) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('kunci/delete_rakor'); ?>",
                    dataType: 'json',
                    data: {
                        id: id
                    },
